...
import com.gsh.hris.service.BenefitsQueryService;
import com.gsh.hris.service.BenefitsService;
...
/**
 * REST controller for managing {@link com.gsh.hris.domain.Benefits}.
 */
@RestController
@RequestMapping("/api")
public class BenefitsResource {
    ...
    public ResponseEntity<BenefitsDTO> updateBenefits(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, benefitsDTO.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/benefits/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<BenefitsDTO> partialUpdateBenefits(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, benefitsDTO.getId().toString())
        );
    }
