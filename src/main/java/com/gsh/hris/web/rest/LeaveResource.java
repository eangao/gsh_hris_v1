...
import com.gsh.hris.service.LeaveQueryService;
...
/**
 * REST controller for managing {@link com.gsh.hris.domain.Leave}.
 */
@RestController
@RequestMapping("/api")
public class LeaveResource {
    ...
    public ResponseEntity<LeaveDTO> updateLeave(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, leaveDTO.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/leaves/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<LeaveDTO> partialUpdateLeave(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, leaveDTO.getId().toString())
        );
    }
