...
import {getTrainingHistoryIdentifier, ITrainingHistory} from '../training-history.model';
...
export class TrainingHistoryService {
  ...
  protected convertDateFromClient(trainingHistory: ITrainingHistory): ITrainingHistory {
    return Object.assign({}, trainingHistory, {
      trainingDate: trainingHistory.trainingDate?.isValid() ? trainingHistory.trainingDate.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.trainingDate = res.body.trainingDate ? dayjs(res.body.trainingDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((trainingHistory: ITrainingHistory) => {
        trainingHistory.trainingDate = trainingHistory.trainingDate ? dayjs(trainingHistory.trainingDate) : undefined;
      });
    }
    return res;
  }
